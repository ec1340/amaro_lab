...
def Get_from_config(c_file):
    ...
    if config.get('RECEPTOR','resname') == 0:
        r_txt = open(r_ind_list,"r")
    ...
    if config.get('LIGAND', 'resname') == 0:
        l_txt = open(l_ind_list,"r")
...
def set_to_MDA():
    ...
    def create_atom_sel(inp_list):
        ...
        atom_sel = u.atoms[inp_list[0]] 

        for i in inp_list[1:]: #skips first item on list
            atom_sel = atom_sel + u.atoms[inp_list[i]]
        return atom_sel
...
def RMSD_of_COM():
    ...
    with open(output_dat_file, 'wb') as handle:
